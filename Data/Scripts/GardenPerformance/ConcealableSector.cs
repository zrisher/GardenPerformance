...
namespace GP.Notes.Concealment.Records {

    public class ConcealableSector {

        #region Subclassses

        [XmlType("ConcealableSectorState")]
        public class SectorState {
            [XmlElement("DisplayName")]
            public String WorldName;
            public String FileName;
            public String GridBuildersFileName;
            public Dictionary<long, ConcealableGrid> ConcealedEntities =
        ...
        private const String STATE_FILENAME_SUFFIX = "_concealment_state";
        private const String GRIDS_FILENAME_SUFFIX = "_concealed_grids";
        ...
        public static bool SaveExistsFor(String worldName, Vector3D sectorPos) {
            return GardenGateway.Files.Exists(FileName(worldName, sectorPos));
        }
        ...
        private SectorState State = new SectorState();
        ...
        public ConcealableSector(String worldName, Vector3D sectorPos) {
            State.WorldName = worldName + sectorPos.ToString();
            State.FileName = State.WorldName + STATE_FILENAME_SUFFIX + FILE_EXT;
            State.GridBuildersFileName = State.WorldName + GRIDS_FILENAME_SUFFIX + FILE_EXT;
        }
        ...
        private void LoadState() {
            ...
            Log.Info("Attempting to loading Concealed Sector State from " + State.FileName, 
                "LoadState");

            //// Empty current state

            if (!GardenGateway.Files.Exists(State.FileName)) {
                Log.Info("No existing state file, starting fresh.", "Load");
                State = new SectorState();
                return;
            }

            String loadedSerializedState = null;
            GardenGateway.Files.Read<String>(State.FileName, ref loadedSerializedState);

            if (loadedSerializedState == null || loadedSerializedState.Length < 1) {
                Log.Error("Existing State file blank, keeping init state.", "Load");
                return;
            }

            SectorState loaded = MyAPIGateway.Utilities.
                SerializeFromXML<SectorState>(loadedSerializedState);
        ...
        private void LoadBuilders() {
            Log.Trace("Loading Builders for sector from " + State.WorldName, "LoadBuilders");
        ...
        private bool ValidateLoaded() {
            ...
            foreach (KeyValuePair<long, ConcealableGrid> kvp in State.ConcealedEntities) {
                ...
                if (!ConcealedBuilders.ContainsKey(kvp.Key)) {
                    ...
                    switch (kvp.Value.Type) {
                        ...
                        case ConcealableEntity.EntityType.Grid:
        ...
        public List<ConcealableGrid> ConcealedGrids() {
            ...
            if (State == null) {
                ...
                return new List<ConcealableGrid>();
            }
            ...
            return State.ConcealedEntities.Values.Select(x => x as ConcealableGrid).
                Where(x => x != null).ToList();
        ...
        public List<ConcealableGrid> RevealedGrids() {
            ...
            List<ConcealableGrid> revealedGrids = gridsList.Select(
                x => new ConcealableGrid() //new RevealedGrid(x)
                ).ToList();
        ...
        private void revealEntity(ConcealableGrid entity) {
            /*
